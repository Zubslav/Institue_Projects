function [p1 , L1, q, p2, L2] = test19(x, calc_full, sigma0,d )
...
for i=1:N 
    if i==1 && x(1,1)==0 && S0<=k %первый шаг, из 0 в 0 
        l(i)= -inf;
        u(i) = (k*(1+d)-S0-i*B)/sigma0; 
         l_array(i) = 0; 
    elseif i==1 && x(1,1)==1 && S0<=k %первый шаг, 0 в 1
        l(i) = (k*(1+d)-S0-i*B)/sigma0;
        u(i) = inf;
        l_array(i) = k*(1+d); 
    elseif i==1 && x(1,1)==0 && S0>=k*(1+d)%первый шаг, 1 в 0
        l(i)= -inf;
        u(i)= (k-S0-i*B)/sigma0; 
        l_array(i) = -k; 
    elseif i==1 && x(1,1)==1 && S0>=k*(1+d) %первый шаг , 1 в 1
        l(i) = (k-S0-i*B)/sigma0;
        u(i) = inf;     
        l_array(i) = 0; 
